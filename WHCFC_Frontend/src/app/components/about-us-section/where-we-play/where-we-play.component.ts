...
export class WhereWePlayComponent implements OnInit {
  title = 'Where We Play';
  ...
  ngOnInit() {
    this.aboutUsDataService.aboutUsData$.subscribe(data => {
      if (data) {
        this.title = data.whereWePlayTitle || this.title;
        this.content = data.whereWePlayContent || this.content;
        if (data.whereWePlayMapUrl) {
          this.mapUrl = this.sanitizer.bypassSecurityTrustResourceUrl(data.whereWePlayMapUrl);
        }
      }
    });
  }
